...
int main (int argc, char *argv[])
{
  ...
  vtkImageReader2 *textureFile =
    readerFactory->CreateImageReader2(argv[2]); // open
  textureFile->SetFileName(argv[2]);            // for read
  textureFile->Update();
  ...
  camera->Azimuth(-45);     // Rotate view up vector centered at the focal point.
  camera->Roll(-90);        // spin camera about its axis
  camera->SetClippingRange(.5, .6);
  double planesArray[24];
  
  // frustum planes 截锥体平面
...
//(1.0/ (vtkMath::RadiansFromDegrees(std::tan(camera->GetViewAngle())))) / 2.0;
  ...
  // vtkCamera vtkPolyData ==> vtkProjectedTexture (projectedTexture) ==> vtkPolyData
  ...
  //pos[2] = pos[2] + 1;
  projectedTexture->SetPosition( pos );
  projectedTexture->SetFocalPoint(camera->GetFocalPoint());
  // set up view vector which is same with camera view up vector.
  ...
  // GetPointData: get Scalars, vectors, etc.
  // Set the texture coordinate data.
  // polyData set texture coordicate data which can found in projectedTexture.
  polyData->GetPointData()->SetTCoords(
    projectedTexture->GetOutput()->GetPointData()->GetTCoords());

  // vtkTexture(texture) <== vtkImageReader2(textureFile) <== vtkImageReader2Factory
  ...
  // vtkCamera ==> vtkPlanes ==> vtkFrustumSource ==> vtkPolyDataMapper ==> vtkActor
  ...
  //renderer->AddActor(frustumActor); // 这看起来没啥用。
  renderer->SetBackground(colors->GetColor3d("SlateGray").GetData());
  renderer->SetActiveCamera(camera);
  camera->Azimuth(180);  //方位角 在z-x（或者说z-y平面内改变）
  camera->Roll(-90);     //转角 （顺时针是负）在(x-y平面内改变)
