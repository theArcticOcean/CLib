...
int main(int argc, char *argv[])
{
  ...
  // https://www.vtk.org/Wiki/VTK/Examples/Python/Visualization/VTKNamedColorPatches_html
  ...
  // vtkTriangleMeshPointNormals is used to compute point normals for a triangle mesh
  ...
  actor->GetProperty()->SetAmbientColor(0.2, 0.2, 1.0);  //环境光颜色
  actor->GetProperty()->SetDiffuseColor(1.0, 0.65, 0.7); //漫反射颜色
  actor->GetProperty()->SetSpecularColor(1.0, 1.0, 1.0); //高光颜色
  actor->GetProperty()->SetSpecular(0.5);   // 光谱系数
  actor->GetProperty()->SetDiffuse(0.7);    // spreading 系数
  actor->GetProperty()->SetAmbient(0.5);    // surrounding 系数
  actor->GetProperty()->SetSpecularPower(20.0); // 光强度
  actor->GetProperty()->SetOpacity(1.0);        // 不透明度
